...
归属：质量收尾RC-05。日期：2026-09-04，执行更新：2026-09-05。状态：RC-05B展示层、共享读层、生产发布及数据库原文纠偏已完成；RC-05C URL/索引最终处置仍待数据。
...
3. 完整build通过后以`73bcebb4`发布；8个生产HTML页面验收通过后执行`--commit`，两条固定记录写入成功并经独立连接回读。迁移只改content/detail，索引判断和质量分保持不变；无需用户执行SQL。
