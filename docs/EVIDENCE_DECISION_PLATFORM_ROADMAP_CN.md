...
| MON-01    | P1     | 30/90 天人工复核闭环           | 事实到期、判断到期和变化待审可筛选；复核依据与缺失基线可见      |   0.5 天 | 代码与生产数据审计完成；事实日历12/12可计算，判断基线7/12，余5项待真实复核；不包含自动抓取 |
...
- OpenRouter于2026-09-06建立独立`decision`判断基线，事件`8b0bcb8c-e836-42ba-8fe0-5236a90c1e00`。依据限定为官方路由、供应
  ...
  `handsOnTrial=false`，重复执行不新增事件。MON判断日历因此变为7/12，时间线事件共14条。
