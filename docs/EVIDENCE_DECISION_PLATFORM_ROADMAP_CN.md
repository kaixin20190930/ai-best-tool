...
| MON-01    | P1     | 30/90 天人工复核闭环           | 事实到期、判断到期和变化待审可筛选；复核依据与缺失基线可见      |   0.5 天 | 代码与生产数据审计完成；工具判断9/10、全部档案9/12，余3项待真实复核；不包含自动抓取       |
...
- Dune的Supabase情报档案此前没有对应Neon目录记录，生产`/ai/dune`实际为404壳。2026-09-06使用固定UUID、域名冲突保护、事务
  ...
  品对比、Data Hub和分享文档建立独立`decision`基线，事件`5fec6387-3667-4e91-a76c-38551fafbd68`。MON现为工具判断9/10、全
  部档案9/12；剩余工具仅The Graph，另2个为非工具档案，不能混称为4个待补工具。
