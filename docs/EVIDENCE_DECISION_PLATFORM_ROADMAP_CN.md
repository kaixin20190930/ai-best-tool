...
| LNK-01    | P1     | Guide / 分类消费统一判断       | 不复制工具事实；链接到对应 Decision Card 和证据                 |     1 天 | 已完成；18个可索引Guide边界、8个关系源和24个分类代表入口通过统一验收，分类入口直达Decision Card |
| MON-01    | P1     | 30/90 天人工复核闭环           | 事实到期、判断到期和变化待审可筛选；复核依据与缺失基线可见      |   0.5 天 | 代码与生产数据审计完成；工具判断10/10；后续到期项待真实复核；不包含自动抓取               |
...
- 原09-02/09-03是n8n/OpenRouter预排期，不是实际发布日；两条实际于09-04迁移，保留原canonical但仍计入索引放行，本周至少12
  次、剩余额度0，保持暂停。偏差及保护见主计划和维护审计。
- 日常发现 10-20 个、深审 4-6 个候选并维护 14-21 个成熟候选缓冲；每天目标新增并公开 2 个、最多 3 个合格成熟工具，新工具默认
  `monitor/noindex`。成熟工具 48-72 小时、普通工具 7-14 天、高风险工具 30 天以上后才可进入独立索引评审；当前每天最多批准
  1 个、目标每周 4 个且硬上限 5 个。没有合格项时允许为 0 并补池，不能降级准入标准。唯一口径见
  [规模化路线图](./CONTENT_SCALE_AND_MAINTENANCE_ROADMAP_2026-09-20_CN.md) 和
...
- 当前 CHG-02 已完成 10/10（2026-09-05至09-06新增Gamma、Luma Dream Machine、n8n、OpenRouter、Runway、Dune、The Graph）。
  每个核心工具均先完成 intelligence 抓取和至少一条人工或有审计记录的owner授权辅助claim核验，再建立基线；不会直接把既有页
  面文案或机器提取结果批量包装为变化历史。
- Gamma 初次官网dry-run发现11个来源但未提取claim；2026-09-05按homepage/pricing/product/help定向重采集后，已建立工具档案
  `63031451-a3eb-497d-9396-d6904aa2d3b3`，共4个官方来源、3条candidate、0冲突。产品名和官网定位待人工核验；定价候选的摘录
  受页面CSS污染，不据此通过或发布价格事实。人工核验前不创建`reviewed_no_change`基线。
...
- Consensus（UUID `f15873ae-c6ef-4f0a-b811-b40c2aba76ab`）已建立第三条公开 `reviewed_no_change` 基线，下次复查日为
  2026-10-02。
- Gamma（工具UUID `6512aa61-8663-49f8-809d-2a2ab4e529ad`）定向采集4个官方来源、3条candidate、0冲突。2026-09-05经owner授
  权辅助审查，通过产品名和“存在公开定价页”，将CSS污染摘录替换为实际页面证据，拒绝当前官网不可复现的旧定位。第4条公开基线
  创建成功，重复执行返回`baseline_already_exists`；下次证据复查为2026-10-05。
- Runway定向dry-run产生6个跨产品定位冲突及可疑价格，不写入档案。Luma Dream Machine定向采集已建立2条candidate、0冲突的档
  案；候选与官方HTML title/description一致，但在正式审核和基线写入前仍不计入完成数。
- Luma后续受控审核只通过与官网description完全一致的定位，拒绝将`Luma Dream Machine | AI Video Generator`整段SEO标题当作
  产品名，并将档案规范名固定为`Luma Dream Machine`。第5条公开基线创建及重复幂等检查通过。
- 初始基线只代表已核验事实，不代表整款产品全面审核。种子脚本统一使用`review_scope=fact`，Gamma/Luma两条既有事件已受控修
  正；摘要明确已核验claim数量，禁止1条事实包装成`full`评审。
- n8n定向采集建立5个官方来源、4条candidate、0冲突的档案。2026-09-05受控审核通过产品名、官网定位、公开价格页存在性和明确
  免费试用4条事实；不从动态缺失内容推断套餐金额。第6条公开基线创建成功，重复执行返回`baseline_already_exists`，下次复查
  为2026-10-05。
- n8n于2026-09-06建立独立`decision`判断基线，事件`1f89e397-7239-4529-b026-39e4d8a05558`。判断依据限定为官方执行计费、
  Community Edition、Sustainable Use License和queue mode资料，不将可自托管表述为零维护或不受限开源；明确
  `handsOnTrial=false`，重复执行不新增事件。
- OpenRouter于2026-09-06建立独立`decision`判断基线，事件`8b0bcb8c-e836-42ba-8fe0-5236a90c1e00`。依据限定为官方路由、供应
  商日志、ZDR和定价页，不将统一接口表述成模型行为一致，也不将ZDR扩大为所有endpoint默认零留存；明确
  `handsOnTrial=false`，重复执行不新增事件。
- Runway于2026-09-06建立独立`decision`判断基线，事件`472944a6-e0aa-4363-8dc7-08d673386e07`。依据限定为官方credits、商业
  使用权、Creative/API分离和Studio编辑资料，不将标称生成量等同于可用成片，也不将Creative credits当成API credits；明确
  `handsOnTrial=false`，重复执行不新增事件。Neon目录ID与Supabase情报owner ID不同的历史映射改由共享解析器按canonical slug
  唯一回查，Evidence Ledger和Change Timeline不再静默缺失。
- Dune的Supabase情报档案此前没有对应Neon目录记录，生产`/ai/dune`实际为404壳。2026-09-06使用固定UUID、域名冲突保护、事务
  锁和回读断言恢复目录记录，状态保持`published + monitor`，因此完整页面可访问但仍`noindex`且不进入sitemap；再根据官方产
  品对比、Data Hub和分享文档建立独立`decision`基线，事件`5fec6387-3667-4e91-a76c-38551fafbd68`。
- The Graph于2026-09-06依据官方产品边界、网络支持、API key安全和查询计费建立独立`decision`基线，事件
  `305ab178-cc0b-4d24-a20f-17320d30309f`；线上页面200、公开时间线可见，并保持`monitor/noindex`。MON现为工具判断10/10、全
  部档案10/12；余下2个是站点与分发项目档案，不能继续算作工具缺口。
- MON适用性于2026-09-06完成收口：所有owner type保留30天事实复核，90天选型判断只适用于`tool`。后台默认Tools范围，并提供
  Site evidence、Distribution history和All profiles切换；站点与暂停分发项目不再生成缺判断基线任务，但事实档案和历史记录
  均保留。运行审计现为事实12/12有排期、适用判断10/10有基线，MON-01完成；不等于已实现自动爬取。
- 本轮真实反例显示站点身份与页面内容曾混用：OpenRouter文档页会改写产品名，Runway产品子页会制造全局定位冲突，n8n FAQ会被
  误判为免费套餐。提取器现将`product_name`和`one_line_positioning`限制为首页事实；套餐名必须来自局部价格卡、问句和比较标
  题被排除，免费套餐必须明确以Free命名。Runway、OpenRouter、n8n复验均为0冲突且0假套餐。
- OpenRouter定向采集建立7个官方来源、3条candidate、0冲突的档案。2026-09-05受控审核通过产品名、官网定位和公开价格页存在
  性；动态价格金额未被提取或发布。第7条公开基线创建成功，重复执行返回`baseline_already_exists`，下次复查为2026-10-05。
- Runway定向采集建立8个官方来源、4条candidate、0冲突的档案。2026-09-05受控审核通过产品名、官网定位和canonical价格页存在
  性，拒绝`/pricing?tab=dev`重复路由证据；第8条公开基线创建及重复幂等检查通过，下次复查为2026-10-05。
- Runway首次持久化前发现同步脚本使用旧Neon连接验证owner、随后写入Supabase，导致当前Supabase中的真实工具被误报不存
  在。owner存在性校验现已与intelligence写入统一到Supabase；校验失败时仍停止写入，不提供绕过参数。
- Dune定向采集建立5个官方来源、3条candidate、0冲突的档案。2026-09-05通过官网定位和公开价格页存在性，拒绝将文档入口
  的`Dune Docs`当作产品名，并将档案规范名固定为`Dune`。第9条公开基线创建及重复幂等检查通过，下次复查为2026-10-05。
- 基线种子主证据已固定为`product_name`优先、`one_line_positioning`次之、其他已验证事实最后，避免无产品名时因数据库返回顺
  序让价格等易变事实随机成为公开基线锚点。
- The Graph定向采集建立7个官方来源、2条candidate、0冲突的档案。2026-09-06通过官网产品名与官网定位，第10条公开基线创建及
  重复幂等检查通过，下次复查为2026-10-05。至此首批10个核心工具变化基线完成，后续转入按`next_review_at`复查而非继续为数量
  扩张。
