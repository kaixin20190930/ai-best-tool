...
| MON-01    | P1     | 30/90 天人工复核闭环           | 事实到期、判断到期和变化待审可筛选；复核依据与缺失基线可见      |   0.5 天 | 代码与生产数据审计完成；事实日历12/12可计算，判断基线6/12，余6项待真实复核；不包含自动抓取 |
...
- n8n于2026-09-06建立独立`decision`判断基线，事件`1f89e397-7239-4529-b026-39e4d8a05558`。判断依据限定为官方执行计费、
  ...
  `handsOnTrial=false`，重复执行不新增事件。MON判断日历因此变为6/12，时间线事件共13条。
