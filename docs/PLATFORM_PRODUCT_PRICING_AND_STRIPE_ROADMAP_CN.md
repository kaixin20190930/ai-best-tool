...
- 当前价格按 Stripe Price 目录执行，不再在页面使用旧的 `$19` Founding Price 文案。
- 年付价格为 `$290/年`，由 Stripe 以 yearly recurring Price 扣款。

#### Founding Agency

当前价格：`$79/月` 或 `$790/年`，仍建议仅邀请制。
...
6. 导入 3 个真实产品并完成 P2。
7. 根据真实使用决定 Pro `$29` 和 Agency `$79` 是否继续，以及是否扩大 Agency 开放范围。
