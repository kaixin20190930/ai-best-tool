...
- 立即措施：本执行任务暂停进一步新增索引批准；不追补 Consensus 或 Gamma 的历史时段，不把额度累积到次日。这是执行约束，不代表后台已有全局技术锁；统一写入保护仍待实现。
- 不自动反复切换已上线页的 index/noindex，不把偏差记录伪装成事后批准。
- 待实现：在生产索引写入入口加入统一的批准日志与日/周额度校验；migration、后台和批量脚本应使用同一规则。现有预审测试只校验单项发布日期和状态，不能保证跨条目额度。
...
| --- | --- | --- | --- | --- | --- |
| MAINT-01 | P0 | 公开收录与索引批准额度分离 | 同日两次索引放行违反内部上限 | 已发现并暂停新增索引；统一写入保护待实现 | Codex；核对本周批准历史 |
