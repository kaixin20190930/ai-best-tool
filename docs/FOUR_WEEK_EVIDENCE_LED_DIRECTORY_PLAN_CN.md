...
| W2-02 | 第 2 周 | 日常收录与候选池 | 每日新增 1-2 条；公开与索引解耦，每天最多 1 条、每周最多 5 条进入索引 | 已完成（首批 14/14；后续持续运营） | Codex + 用户 |
...
- 每日下午：处理 3-5 个候选并维持至少 7 个成熟候选缓冲，每天新增并公开至少 1 个、最多 2 个通过门槛的成熟工具；新页默认 `monitor / noindex`。2026-09-09 起，成熟工具经过 48-72 小时技术观察可进入索引评审，当前每天最多 1 个、每周最多 3 个进入 sitemap，数据库每周 5 个仅作硬保护；唯一口径以 [索引发布政策](./TOOL_INDEX_RELEASE_POLICY_CN.md) 为准。
