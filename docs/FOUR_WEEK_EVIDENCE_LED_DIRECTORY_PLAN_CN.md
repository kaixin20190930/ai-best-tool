...
| W1-01B | 收口正文“快速决策”和侧栏重复字段；保留一个主卡 + 一个纯信任摘要 | 详情页 JSX | 1 天 | W1-01A | build + 页面结构断言 | 进行中 |
| W1-01C | 提取统一 Decision Card 数据结构，避免页面区块各自推断 | 详情 presenter / helper | 1 天 | W1-01B | 单元脚本 + build | 待执行 |
| W1-02A | 定义证据完整度：来源、核查日、限制、素材、适合/不适合、比较路径 | service / admin types | 0.5 天 | W1-01C | 类型检查 | 待执行 |
| W1-02B | 前台展示最后核查、下次事实复查、下次判断复核及待补项 | 详情页 | 1 天 | W1-02A | 页面结构断言 | 待执行 |
| W1-03A | 建立候选/准入台账，记录处理结果和缺失原因 | admin action / page | 1 天 | W1-02A | 回归脚本 | 待执行 |
| W1-04A | 新增本地详情页 Decision Card 自动测试 | `scripts/`、`package.json` | 0.5 天 | W1-01C | 脚本自身通过 | 待执行 |

### 第 2 周：后台工作流与日常收录（09-08 至 09-14，预计 5 个开发日）

| 子任务 | 实施细节 | 预计时间 | 依赖 | 验收 | 状态 |
| --- | --- | ---: | --- | --- | --- |
| W2-01A | 后台列表增加完整度、准入状态、下次复查、缺失项筛选 | 1.5 天 | W1-02 | 管理员一屏识别能否发布 | 待执行 |
| W2-01B | 编辑页增加证据来源、限制和 Decision Card 字段校验 | 1.5 天 | W2-01A | 缺字段无法误标可发布 | 待执行 |
| W2-02A | 生成未来 3 天候选池，每日处理 1-2 条 | 每日 1 小时 | W2-01 | 每条有发布/待补结论 | 待执行 |
| W2-02B | 首批 7-14 条处理台账；只发布通过准入的条目 | 每日 1 小时 | W2-02A | 无低质量例外放行 | 待执行 |
...
| 子任务 | 实施细节 | 预计时间 | 依赖 | 验收 | 状态 |
| --- | --- | ---: | --- | --- | --- |
| W3-01A | AI 检测官网、定价、文档变化，只创建待审差异 | 1.5 天 | W1-02 | 不自动覆盖已核验事实 | 待执行 |
| W3-01B | 30 天事实复查与 90 天判断复核队列 | 1 天 | W3-01A | 到期状态可筛选 | 待执行 |
| W3-02A | owner 更新、纠错、评论映射到具体工具证据与判断 | 1 天 | W2-01B | 变更可追溯，不自动发布 | 待执行 |
| W3-03A | 增强 5 个已有曝光详情页 | 1-1.5 天 | W1-02 | 每页 2 个官方来源 + 1 个限制 | 待执行 |
