...
阶段产能、维护频率和一周至一年目标统一见 [高质量工具规模化与分层维护路线图](./CONTENT_SCALE_AND_MAINTENANCE_ROADMAP_2026-09-20_CN.md)。

- 维持 14-21 个完成预审的成熟候选缓冲池；常态每天目标新增并公开 2 个、最多 3 个通过全部公开门槛的成熟工具，数据库默认状态为 `monitor`。
- “目标每天 2 个”是候选准备和编辑产能目标，不是降级质量标准的授权。当天确实没有合格项时允许为 0，并记录异常、补充候选池，不得发布弱页凑数。
- 索引与公开彻底分离：当前运营上限为每天最多批准 1 个、目标每周 4 个；数据库每周 5 个的硬保护上限继续保留，不能自动当作配额用满。
