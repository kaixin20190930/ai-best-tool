...
| MEASURE-01 | 决策事件隐私基础层 | 固定事件/字段 allowlist、30 分钟易失 flow、服务端幂等、流量排除、默认关闭、最小权限未执行迁移与 SEO 零差分 | `PROD_VERIFIED / CLOSED`；[专项文档](./MEASURE_01_DECISION_EVENT_FOUNDATION_CN.md)，独立 QA、主分支 build 与生产 smoke 通过；生产采集仍关闭，迁移、保留期与数据治理待 Owner 批准 | Codex + Owner |
| MEASURE-02 | 数据治理与 Pilot 边界 | 原始 35 天、聚合 400 天、审计 90 天、20-flow 最小样本、最小权限、内部流量轮换和会议任务簇 Pilot allowlist | `IMPLEMENTED / DISABLED`；[治理与 Pilot 文档](./MEASURE_02_DATA_GOVERNANCE_AND_PILOT_CN.md)及未执行迁移已形成；未接 UI、未执行迁移、未开启生产采集 | Codex + Owner |
