...
- P1：CHG-02 已推进至6/10。Gamma、Luma Dream Machine与n8n本轮完成受控官网核验及幂等`fact`基线；n8n通过产品名、官网定位、公开价格页和免费试用4条事实。提取器已限制站点身份事实只取首页，并拦截FAQ/比较标题假套餐；Runway/OpenRouter/n8n真实dry-run均由冲突或假套餐恢复为0，未把缺失动态价格写成事实。Perplexity/Make按robots停止，Poe证据过薄不凑数。
...
| CHG-02 | 首批核心工具变化基线               | 10-20 个核心工具拥有真实基线复核；没有变化时只记录 `reviewed_no_change`，禁止伪造变化              | 进行中（6/10）；Fathom、Claude、Consensus、Gamma、Luma Dream Machine、n8n 已完成；其余候选继续受证据质量与 robots 门禁约束      | Codex + 用户    |
