...
- 不批量新增同义 guide、comparison 或 alternatives URL。工具候选每天处理 3-5 个，常态公开 2-3 个、单周最多 15 个；候选处理或创建 draft 不计入公开日更。只有同时通过资料完整度与独立市场验证的条目才公开，未通过的只保留 draft / `noindex`。
...
| IDX-06 | 高质量收录候选池与 "Best Decision Card" | Claude/Anthropic、Fathom、Gamma、Consensus、DeepL、Runway、Luma AI、Pipedream、Cursor 已完成实体迁移或决策信号收口；下一项为 P1 The Graph | 进行中（成熟工具队列 9/10） | Codex + 用户 |
...
| CTR-06 | 首批成熟工具内容缺口 | Claude/Anthropic、Fathom、Gamma、Consensus、DeepL、Runway、Luma AI、Pipedream、Cursor、The Graph | 10 个既有 canonical URL 完成合并、数据库迁移或决策内容增强；前 9 项已完成且未新增 canonical URL | 进行中（9/10） | Codex |
