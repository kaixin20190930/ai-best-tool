...
| MEASURE-03 | Finder 事件最小接入 | 六类既有 allowlist 事件接入，未配置时零请求；敏感条件只发 `redacted`，不改变页面与 SEO | `LOCAL_VERIFIED / DORMANT / COLLECTION_DISABLED`；Owner 决定暂不配置环境变量，代码保持休眠，详见 [MEASURE-03](./MEASURE_03_DORMANT_UI_INTEGRATION_CN.md) | Codex |
