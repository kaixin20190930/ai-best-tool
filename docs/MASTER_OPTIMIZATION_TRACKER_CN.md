...
执行状态：进行中；新增索引批准暂缓，先核对公开收录与索引额度分离。
...
- P0：发现同日 OpenRouter 与 n8n 两次索引放行违反每日最多 1 个的内部策略。公开上限 2 个不能替代索引上限。统一数据库保护已应用生产，默认暂停，覆盖后台/迁移/批量写入；并发、日/周额度及回滚测试、完整 build 通过。本周批准历史核对仍未完成，不恢复批准。此前两页部署/质量验收通过不代表节奏合规。操作见 [保护运行说明](./INDEX_RELEASE_GUARD_RUNBOOK_CN.md)。
- P0：本轮生产 SEO、health 和 ads.txt 只读审计通过；sitemap 162 URLs。另修复 SEO smoke 重定向请求无超时/未释放响应体的问题，重跑断言全部通过且退出 0。
- P1：Consensus 与 Gamma 到期复核；23 个历史 published 工具缺少显式下次复核日期，先核验再补字段，不批量刷新“已验证”日期。
