...
- P1：CHG-02 已推进至8/10。Gamma、Luma Dream Machine、n8n、OpenRouter与Runway本轮完成受控官网核验及幂等`fact`基线；Runway通过3条独立事实并拒绝1条价格页URL变体。同步脚本已统一从Supabase验证工具owner，消除旧Neon与当前档案错库导致的假“不存在”。提取器继续限制站点身份只取首页并拦截FAQ/比较标题假套餐；Perplexity/Make按robots停止，Poe证据过薄不凑数。
...
| CHG-02 | 首批核心工具变化基线               | 10-20 个核心工具拥有真实基线复核；没有变化时只记录 `reviewed_no_change`，禁止伪造变化              | 进行中（8/10）；Fathom、Claude、Consensus、Gamma、Luma Dream Machine、n8n、OpenRouter、Runway 已完成；其余候选继续受证据质量与 robots 门禁约束 | Codex + 用户    |
