...
- P1：CHG-02 已推进至5/10。Gamma通过产品名/定价页并拒绝旧定位；Luma Dream Machine通过官网定位、拒绝带SEO后缀的产品名并规范档案名。两条`reviewed_no_change`均已建立且重复运行不新增，范围从误导性的`full`修正为`fact`。Runway/OpenRouter跨产品冲突未写入，Perplexity/Make按robots停止，n8n价格误识别未写入；Poe只提取定价页，暂不以单薄证据凑数。
