...
- 不批量新增同义 guide、comparison 或 alternatives URL。成熟候选保持至少 7 条缓冲，每天新增并公开至少 1 个、最多 2 个通
  过全部门槛的成熟工具，新工具默认 `monitor / noindex`。成熟工具在 48-72 小时技术观察后可进入独立索引评审；当前每天最多
  批准 1 个、每周最多 3 个，数据库每周 5 个上限仅作硬保护。只有同时通过资料完整度、独立市场验证和索引复核的条目才进入
...
| MEASURE-01 | 决策事件隐私基础层 | 固定事件/字段 allowlist、30 分钟易失 flow、服务端幂等、流量排除、默认关闭、最小权限未执行迁移与 SEO 零差分 | `PROD_VERIFIED / CLOSED`；[专项文档](./MEASURE_01_DECISION_EVENT_FOUNDATION_CN.md)，独立 QA、主分支 build 与生产 smoke 通过；生产采集仍关闭，治理边界已转入 MEASURE-02 | Codex + Owner |
| MEASURE-02 | 数据治理与 Pilot 边界 | 原始 35 天、聚合 400 天、审计 90 天、20-flow 最小样本、最小权限、内部流量轮换和会议任务簇 Pilot allowlist | `QA_PASS / DISABLED`；[治理与 Pilot 文档](./MEASURE_02_DATA_GOVERNANCE_AND_PILOT_CN.md)，首轮 4 项 P1 已修复并由同一 QA 复验通过；未接 UI、未执行迁移、未开启生产采集 | Codex + Owner |
