...
- P1：CHG-02 已推进至7/10。Gamma、Luma Dream Machine、n8n与OpenRouter本轮完成受控官网核验及幂等`fact`基线；OpenRouter只通过产品名、官网定位和公开价格页存在性，不从动态页面发布金额。提取器已限制站点身份事实只取首页，并拦截FAQ/比较标题假套餐；Runway/OpenRouter/n8n真实dry-run均由冲突或假套餐恢复为0。Perplexity/Make按robots停止，Poe证据过薄不凑数。
...
| CHG-02 | 首批核心工具变化基线               | 10-20 个核心工具拥有真实基线复核；没有变化时只记录 `reviewed_no_change`，禁止伪造变化              | 进行中（7/10）；Fathom、Claude、Consensus、Gamma、Luma Dream Machine、n8n、OpenRouter 已完成；其余候选继续受证据质量与 robots 门禁约束 | Codex + 用户    |
