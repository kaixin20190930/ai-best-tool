...
| IDX-06 | 高质量收录候选池与 "Best Decision Card" | 市场验证机制已完成；Claude/Anthropic、Fathom、Gamma 已完成实体迁移和信号收口，下一项为 Consensus 数据库迁移与未收录修复 | 进行中（成熟工具队列 3/10） | Codex + 用户 |
...
| CTR-06 | 首批成熟工具内容缺口 | Claude/Anthropic、Fathom、Gamma、Consensus、DeepL、Runway、Luma AI、Pipedream、Cursor、The Graph | 10 个既有 canonical URL 完成合并、数据库迁移或决策内容增强；前三项已完成且未新增 URL | 进行中（3/10） | Codex |
