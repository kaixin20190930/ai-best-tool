...
- 不批量新增同义 guide、comparison 或 alternatives URL。工具候选每天处理 3-5 个，常态公开 2-3 个、单周最多 15 个；候选处
  理或创建 draft 不计入公开日更。只有同时通过资料完整度与独立市场验证的条目才公开，未通过的只保留 draft / `noindex`。
...
| CHG-02 | 首批核心工具变化基线               | 10-20 个核心工具拥有真实基线复核；没有变化时只记录 `reviewed_no_change`，禁止伪造变化               | 进行中（2/10）；Fathom、Claude 官方来源基线已写入并回读；Gamma 动态站待定向采集                                            | Codex + 用户    |
