...
| 登录入口重复语言前缀 | 新发现，待定位修复 | Codex；独立工作项，不依赖 GSC |

附带问题：四个工具页实际 href 出现 `/en/en/login?redirect=/en/ai/...` 或 `/cn/cn/login?redirect=/cn/ai/...`。目前仅确认链接异常，尚未验证登录后的跳转结果。后续检查本地化 Link 与 href 拼接边界，测试中英文入口及登录返回路径；不在本次对象审计中顺手修改认证逻辑。
