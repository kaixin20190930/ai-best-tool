...
| Week 7 | 2026-08-31 | 2,243 | 13 | 0.58% | 32.07 | 167 | 1,234 | 性能窗口为 08-01 至 08-28；后 14 天贡献 2,155 展示 / 13 点击。增长强烈集中在首页（1,838 展示），但已从纯观察转为保护增长、扩散到少数非首页机会页。Coverage 图表最新点为 08-21。 |
| Week 8 | 2026-09-21 | 2,837 | 19 | 0.67% | 28.97 | 140 | 1,363 | 28 天总量改善，但后 14 天仅 476 展示 / 2 点击，最近 7 天 170 / 0；已抓取未收录升至 45。保持索引暂停，详见 [09-21 复盘](./GSC_REVIEW_2026-09-21_CN.md)。 |

## 2026-09-21 Coverage 更新

| 问题 | 网页数 | 相比 08-31 | 处理 |
| --- | ---: | ---: | --- |
| 被 `noindex` 标记排除 | 1,222 | +107 | 继续主动收口，不批量开放 |
| 已抓取 - 尚未编入索引 | 45 | +21 | 等待 URL drilldown，逐条分类 |
| 网页会自动重定向 | 45 | +11 | 生产重定向通过，观察旧 URL 重抓 |
| 备用网页（适当 canonical） | 33 | -1 | 基本稳定 |
| 软 404 | 13 | -9 | 改善，仍需 URL 明细 |
| 重复网页，用户未选定 canonical | 5 | +1 | 等待 URL 明细定位 |
...
来自 `aibesttool.com-Coverage-2026-08-31.xlsx` 的关键问题分布（Coverage 图表最新点为 2026-08-21）：

| 问题 | 网页数 | 备注 |
| --- | --- | --- |
| 被 `noindex` 标记排除 | 1,115 | 主要是主动收口的弱页 / 非索引页；不应因数量上升而批量放开索引 |
| 已抓取 - 尚未编入索引 | 24 | 下一轮逐 URL 分类：保留增强、canonical/noindex 或技术修复 |
| 网页会自动重定向 | 34 | 多半是规范化 / 域名跳转带来的结果，继续监控 |
| 备用网页（有适当的规范标记） | 34 | 规范化仍在生效；避免把变体加入 sitemap 或内链主路径 |
| 软 404 | 22 | 需要核对是否为无实质内容的工具/旧路径并选择 404、合并或补实质内容 |
| 重复网页，用户未选定规范网页 | 4 | 需要继续收口同义页和 alias 页 |
| 重复网页，Google 选择的规范网页与用户指定的不同 | 1 | 关注该单一 URL 的 canonical 与内容一致性 |
...
- 28 天展示从 147 增至 2,243，点击从 0 增至 13，平均排名从 62.59 改善至 32.07。最近 14 天贡献约 96% 的展示，变化发生在窗口后半段。
- 首页有 1,838 展示，占页面表展示约 81.9%。增长的核心仍是 `ai tools directory`、`ai tool directory`、`ai top tools` 等目录词，说明 Google 开始更稳定地理解首页定位。
- 非首页已经出现真实机会，但仍然很薄：Web3 guide 52、Automation guide 40、Fathom 55、Anthropic 26、DeepL 16、Gamma 12 展示。它们应优先做真实证据、意图和站内路径增强，而非创建同义新页。
- `noindex` 1,115 条是主动收口带来的预期结果；需处理的是 24 个已抓取未编入索引、22 个软 404、4 个重复未指定 canonical 和 1 个 Google canonical 不一致 URL。

### 下轮动作

1. 逐 URL 审计 Coverage 异常项，给每条明确保留、合并、noindex、404 或技术修复决策。
2. 审核首页目录词 snippet 与首页到 Explore/分类的分流，避免所有目录意图都只停留在首页。
3. 只增强已有展示的 3 个 Guide 与 5 个工具详情页；每项必须有官方来源、核查日期和真实选择边界。
4. 7 天后同时对比 7 天与 28 天数据，重点看非首页展示份额、产生展示的页面数，以及目录词 CTR 是否继续改善。
