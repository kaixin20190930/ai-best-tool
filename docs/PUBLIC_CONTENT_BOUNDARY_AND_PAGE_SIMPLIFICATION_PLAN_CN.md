...
状态：PUB-01 至 PUB-04 均已完成独立 QA、部署与生产验证并关闭；真实 Pilot 作为后续独立实验，尚未启动
...
状态：`PROD_VERIFIED / CLOSED`。fail-closed 注册、服务端展示门禁、回滚保留数据语义和自动测试已通过独立 QA、本地主分支完整 build 与生产 SEO smoke；生产注册、Pilot 页面与实验状态仍为空/关闭。未选择首个任务簇，未新增 URL、分析事件或 SEO 变更。
