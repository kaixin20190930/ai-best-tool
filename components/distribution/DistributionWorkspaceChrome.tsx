...
import { Menu, Home, Package, ListChecks, Wrench, BellRing, BarChart3, Settings, ArrowLeft, ArrowRight } from 'lucide-react';
...
const workspaceItems = [
  ...
  { id: 'opportunities', href: '/distribution/opportunities', label: '目标机会', description: '推荐与决策', icon: ListChecks },
  { id: 'tasks', href: '/distribution/tasks', label: '执行任务', description: '提交与阻塞处理', icon: Wrench },
  { id: 'monitoring', href: '/distribution/monitoring', label: '跟进与监控', description: '复查与异常', icon: BellRing },
  { id: 'reports', href: '/distribution/reports', label: '结果报告', description: '指标与复盘', icon: BarChart3 },
];

const extraItems = [{ id: 'settings', href: '/distribution/settings', label: '套餐与设置', description: '升级与偏好', icon: Settings }];
              ...
              {workspaceItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <Icon className='h-4 w-4' />
                    <span className='font-semibold'>{item.label}</span>
                    <span className='ml-auto text-[11px] text-slate-500'>{item.description}</span>
                  </Link>
                );
              })}
            </nav>
              ...
              {extraItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    href={withQuery(item.href)}
                    aria-label={`${locale === 'cn' ? '进入' : 'Open'} ${item.label}`}
                    className={`mt-2 flex items-center gap-2 rounded-xl border px-3 py-2.5 text-sm ${
                      activeSection === item.id
                        ? 'border-cyan-300 bg-cyan-50 text-cyan-900'
                        : 'border-slate-200 bg-white text-slate-700 hover:border-slate-300 hover:bg-slate-50'
                    }`}
