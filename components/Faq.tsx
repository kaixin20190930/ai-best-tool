...
function TitleItem({ children }: { children: React.ReactNode }) {
  return (
    <h2 className='flex items-center gap-1 text-2xl text-blue-600'>
      <CircleHelp /> {children}
    </h2>
  );
}
...
function ContentItem({ children }: { children: React.ReactNode }) {
  return <h3 className='mt-3 text-blue-500'>{children}</h3>;
}
...
export default function Faq() {
  ...
  return (
    <div className='mx-auto max-w-pc space-y-8 pb-5'>
      <h1 className='text-center text-2xl font-bold text-blue-700 lg:pb-3 lg:text-3xl'>{t('title')}</h1>
