...
function TitleItem({ children }: { children: React.ReactNode }) {
  return (
    <h2 className='flex items-center gap-2 text-xl font-semibold text-slate-900 lg:text-2xl'>
      <CircleHelp className='size-5 text-cyan-700' /> {children}
    </h2>
  );
}

function ContentItem({ children }: { children: React.ReactNode }) {
  return <h3 className='mt-3 text-sm leading-6 text-slate-600 lg:text-base'>{children}</h3>;
}
...
export default function Faq() {
  ...
  return (
    <div className='mx-auto max-w-pc space-y-8 pb-5'>
      <h1 className='text-center text-2xl font-bold text-slate-950 lg:pb-3 lg:text-3xl'>{t('title')}</h1>
      <div className='grid grid-cols-1 gap-5 px-3 lg:grid-cols-2 lg:gap-16 lg:px-0'>
        <div className='theme-surface rounded-lg p-5 lg:p-6'>
