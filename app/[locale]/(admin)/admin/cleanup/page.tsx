...
export default async function AdminCleanupPage({
  ...
  const currentBlockerLabel = blockerFilter !== 'all' ? blockerFilter : topBlocker?.label || availableBlockers[0] || '';
      ...
      {priorityBlockers.length > 0 && (
        <section className='rounded-[20px] border border-slate-200 bg-white p-5 shadow-sm'>
          <div className='flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between'>
            <div>
              <h2 className='text-lg font-semibold text-slate-950'>{isChinese ? '优先修复顺序' : 'Priority order'}</h2>
      ...
      {blockerGroups.length > 0 && (
        <section className='overflow-hidden rounded-[20px] border border-slate-200 bg-white shadow-sm'>
          <div className='border-b border-slate-200 px-5 py-4'>
            <h2 className='text-lg font-semibold text-slate-950'>{isChinese ? '按 blocker 分组' : 'Blocker groups'}</h2>
            <p className='mt-1 text-sm leading-6 text-slate-600'>
              {isChinese
      ...
      <section className='overflow-hidden rounded-[20px] border border-slate-200 bg-white shadow-sm'>
        <div className='border-b border-slate-200 px-5 py-4'>
          <div className='flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between'>
            <div>
              <h2 className='text-lg font-semibold text-slate-950'>{isChinese ? '待处理条目' : 'Items to clean up'}</h2>
              <p className='mt-1 text-sm leading-6 text-slate-600'>
                {isChinese
                  ? '先补齐阻塞字段，再回到工具编辑页保存。'
