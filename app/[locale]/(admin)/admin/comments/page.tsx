...
export default async function AdminCommentsPage({
  ...
  const status =
    searchParams?.status === 'visible' || searchParams?.status === 'hidden'
      ? searchParams.status
      : 'all';
  ...
  const priorityLabel =
    summary.unresolvedReportedComments > 0
      ? 'Resolve unresolved reports first'
      : summary.autoHiddenComments > 0
      ? 'Review auto-hidden comments'
      : 'Moderation queue is in good shape';
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-slate-600">
              Review user comments and hide spam or low-quality content.
            </p>
          </div>
          <form
            action={async () => {
        ...
        {[
          ...
          <Link
            ...
            href={
              item.key === 'all'
                ? status === 'all'
                  ? `/admin/comments?sort=${sort}`
                  : `/admin/comments?status=${status}&sort=${sort}`
                : status === 'all'
                ? `/admin/comments?sort=${sort}&reportState=unresolved`
                : `/admin/comments?status=${status}&sort=${sort}&reportState=unresolved`
            }
            className={`rounded-lg px-3 py-1.5 text-sm font-medium ${
