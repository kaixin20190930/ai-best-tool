...
export default async function AdminAnalyticsPage({
  ...
  try {
    ...
    const getPageTrendSummary = (pageType: string) => {
      ...
      if (pageType === 'tool_detail')
        return 'If detail pages are rising, trust signals and feedback loops matter most.';
      if (pageType === 'explore') return 'If Explore is rising, tighten filters and empty-state guidance.';
      if (pageType === 'best_ai_tools')
        return 'If top lists are rising, keep the hub focused and the topic paths clear.';
    ...
    const outreachStatusRows = [
      ...
      },
      {
        key: 'closed',
        label: 'Closed',
        count: outreachQueue.filter((item) => item.outreachStatus === 'closed').length,
      },
    ].map((item) => ({
      ...item,
    ...
    const highIntentPageFamilyRows = [...highIntentPageFamilies]
      .sort(
        (a, b) => b.downstreamConversions - a.downstreamConversions || b.ctaClicks - a.ctaClicks || b.views - a.views,
      )
      .slice(0, 6);
        ...
        <div className='mb-8'>
          <div className='mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between'>
            ...
            <div className='text-sm text-slate-500'>
              {commercialIntentReport.sources.length > 0
                ? 'Top source is highlighted below'
                : 'Waiting for funnel data'}
            </div>
          </div>
        ...
        <div className='mb-8'>
          ...
          <div className='mb-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4'>
            <div className='rounded-lg border border-slate-200 bg-white p-5 shadow-sm'>
              ...
              <p className='mt-2 text-3xl font-semibold text-slate-900'>
                {ctaClickReport.totalClicks.toLocaleString()}
              </p>
        ...
        <div className='mb-8'>
          ...
          <div className='mb-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4'>
            <div className='rounded-lg border border-slate-200 bg-white p-5 shadow-sm'>
              ...
              <p className='mt-2 text-3xl font-semibold text-slate-900'>
                {ctaClickTrend.currentClicks.toLocaleString()}
              </p>
        ...
        <div className='mb-8'>
          ...
          <div className='mb-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3'>
            ...
            <div className='rounded-lg border border-slate-200 bg-white p-5 shadow-sm'>
              ...
              <p className='mt-2 text-2xl font-semibold text-slate-900'>
                {topCtaRateFamily
                  ? `${topCtaRateFamily.label} · ${topCtaRateFamily.viewToCtaRate}%`
                  : 'Waiting for data'}
              </p>
              <p className='mt-2 text-sm text-slate-500'>
                {topCtaRateFamily
                      ...
                      <tr key={item.pageType} className='align-top'>
                        ...
                        <td className='px-4 py-4'>
                          <div className='min-w-[150px] text-xs leading-5 text-slate-500'>
                            <p>
                              View -&gt; CTA:{' '}
                              <span className='font-semibold text-slate-900'>{item.viewToCtaRate}%</span>
                            </p>
                            <p className='mt-1'>
                              View -&gt; downstream:{' '}
                        ...
                        <td className='px-4 py-4 text-xs leading-5 text-slate-500'>
                          {getPageFamilyFunnelAction(item)}
                        </td>
                        <td className='px-4 py-4'>
            ...
            {pageSummaryItems.map((item) => (
              <div key={item.pageType} className='rounded-lg border border-slate-200 bg-white p-5 shadow-sm'>
                ...
                <div className='mt-3 text-sm text-slate-600'>
                  {item.uniqueVisitors.toLocaleString()} unique visitors
                </div>
                ...
                <div className='flex items-start justify-between gap-3'>
                  <div>
                    ...
                    <div className='mt-2 text-2xl font-semibold text-slate-900'>
                      {item.currentViews.toLocaleString()}
                    </div>
          ...
          <div className='mt-4 rounded-lg border border-cyan-100 bg-cyan-50/60 p-5 shadow-sm'>
            <div className='flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between'>
              ...
              <p className='max-w-2xl text-sm leading-6 text-slate-600'>
                This turns the page report into a short work queue: where traffic is already showing up, where to
                deepen, and which page family should get the next pass.
              </p>
            </div>
                ...
                {topCategories.map((category, index) => (
                  <tr key={category.id} className={index === 0 ? 'bg-cyan-50/30' : ''}>
                    ...
                    <td className='px-4 py-4 text-right text-sm text-slate-700'>
                      {category.favorites.toLocaleString()}
                    </td>
                    <td className='px-4 py-4 text-right text-sm text-slate-700'>
                      {category.comments.toLocaleString()}
                    </td>
        ...
        <div className='mb-8'>
          <div className='mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between'>
            <div>
              ...
              <p className='mt-1 text-sm text-slate-600'>
                Which source pages are actually sending owners into claim submission or paid checkout, not just
                collecting shallow CTA clicks.
              </p>
            </div>
            <Link
          ...
          <div className='mb-4 rounded-lg border border-cyan-200 bg-cyan-50 p-5 shadow-sm'>
            <div className='flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between'>
              <div>
                ...
                <p className='mt-1 text-sm text-slate-600'>
                  What happened after outreach-driven claims: did they stay free, enter the paid path, confirm payment,
                  or turn into featured inventory?
                </p>
              </div>
              <p className='text-xs text-slate-500'>
            ...
            <p className='mt-3 text-xs text-slate-500'>
              {outreachPaidPlanRate}% of outreach-led claims are on the paid plan and {outreachPaymentConfirmedRate}%
          ...
          <div className='mb-4 grid gap-6 lg:grid-cols-[1.2fr_0.8fr]'>
            <div className='overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm'>
              ...
              <div className='divide-y divide-slate-100'>
                {outreachStatusRows.length > 0 ? (
                  ...
                  <div className='px-4 py-8 text-sm text-slate-500'>
                    No outreach leads are visible in the queue yet.
                  </div>
                )}
              </div>
            </div>
            <div className='grid gap-4 sm:grid-cols-3 lg:grid-cols-1'>
              ...
              <div className='theme-surface rounded-lg border border-slate-200 p-6 shadow-sm'>
                ...
                <p className='mt-2 text-sm text-slate-500'>
                  Historical close rate across all tracked outreach leads, including listings that were later claimed
                  and left the active queue.
                </p>
    ...
    console.error('Admin analytics page failed to load:', error);
    return (
      <div className='space-y-6'>
        <div className='rounded-2xl border border-rose-200 bg-rose-50 p-6 text-rose-900'>
          <h1 className='text-2xl font-bold'>Analytics unavailable</h1>
          <p className='mt-2 text-sm leading-6'>
            The analytics dashboard could not finish loading right now. You can refresh the page or come back later.
          </p>
