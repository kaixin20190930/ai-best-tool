...
function CollectionFallback({ error }: { error: string }) {
  return (
    <div className='space-y-4'>
      <div className='rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800'>
        Collection queue loaded with partial data: {error}
      </div>
      <div className='theme-surface rounded-lg border border-slate-200 p-6 shadow-sm'>
        ...
        <p className='mt-2 text-slate-600'>
          Some collection services are unavailable right now. Retry the page after the failing source recovers.
        </p>
...
const validCandidateScoreFilters: CollectionCandidateScoreFilter[] = ['all', 'promising', 'low'];
...
function getCategoryAction(category: { toolCount: number; opportunityScore: number; views: number; comments: number }) {
...
export default async function AdminCollectionPage({
  ...
  try {
    ...
    if (results[4].status === 'fulfilled') {
      ...
      loadError =
        results[4].reason instanceof Error ? results[4].reason.message : 'Failed to load collection candidates.';
    }
      ...
      <div>
        ...
        <div className='theme-surface mt-6 overflow-hidden rounded-lg border border-slate-200 shadow-sm'>
          <div className='border-b border-slate-200 px-6 py-4'>
            ...
            <p className='mt-1 text-sm text-slate-600'>Manual and scheduled collection run history.</p>
          </div>
          <div className='divide-y divide-slate-200'>
            {runs.map((run) => (
              ...
              >
                <div>
                  <p className='font-medium text-slate-900'>{run.source_name || 'Unknown source'}</p>
            ...
            {runs.length === 0 && (
              <div className='px-6 py-10 text-center text-sm text-slate-500'>No collection runs yet.</div>
            )}
          </div>
        </div>
        ...
        <div className='theme-surface mt-6 overflow-hidden rounded-lg border border-slate-200 shadow-sm'>
          <div className='border-b border-slate-200 px-6 py-4'>
            ...
            <p className='mt-1 text-sm text-slate-600'>{queue.length} items waiting for research or review</p>
          </div>
    ...
    console.error('Failed to load admin collection page:', error);
    return <CollectionFallback error={error instanceof Error ? error.message : 'Unknown error'} />;
  }
}
