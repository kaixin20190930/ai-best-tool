        ...
        items={[
          {
            ...
            value: locale === 'cn' || locale === 'tw' ? '能否自定义得足够深' : 'Can it be customized deeply enough',
            note:
              locale === 'cn' || locale === 'tw'
                ? 'n8n 的替代项如果控制力不够，意义会弱很多。'
                : 'If the alternative lacks control, it is not much of an alternative.',
          },
          {
            label: locale === 'cn' || locale === 'tw' ? '可维护性' : 'Maintainability',
            value: locale === 'cn' || locale === 'tw' ? '团队能否长期维护' : 'Can a team maintain it long term',
            note:
              locale === 'cn' || locale === 'tw'
                ? '自动化最后拼的是长期维护，而不是一时搭好。'
                : 'Automation wins by being maintainable, not just by being built once.',
          },
          {
            label: locale === 'cn' || locale === 'tw' ? '开发者适配' : 'Developer fit',
            value: locale === 'cn' || locale === 'tw' ? 'API 和脚本是否顺手' : 'Are APIs and scripts natural to use',
            note:
              locale === 'cn' || locale === 'tw'
                ? '如果工作流已经深到 API 和脚本，这一点会很关键。'
      ...
      <section className='mt-6 grid gap-4 rounded-[18px] border border-cyan-200 bg-cyan-50/70 p-6 shadow-sm md:grid-cols-3'>
        <div>
          ...
          <p className='mt-2 text-sm leading-6 text-slate-600'>
            {locale === 'cn' || locale === 'tw'
              ? '这页已按真实 n8n 替代路径重新核对，保留控制、维护和开发者入口。'
              : 'This page has been rechecked against a real n8n-alternative workflow and keeps control, maintainability, and developer entry points visible.'}
          </p>
        </div>
        <div>
          <p className='text-xs font-semibold uppercase tracking-wide text-cyan-700'>
          ...
          <p className='mt-2 text-sm leading-6 text-slate-600'>
            {locale === 'cn' || locale === 'tw'
              ? '用控制力、可维护性和真人评论把它和泛自动化页区分开。'
              : 'Use control, maintainability, and real comments to differentiate it from generic automation pages.'}
          </p>
        </div>
        <div>
          <p className='text-xs font-semibold uppercase tracking-wide text-cyan-700'>
          ...
          <p className='mt-2 text-sm leading-6 text-slate-600'>
            {locale === 'cn' || locale === 'tw'
              ? '后续优先补案例、脚本样例和真人评论。'
              : 'Next, prioritize cases, script examples, and real comments.'}
          </p>
        </div>
      </section>
      <section className='mx-auto mt-8 max-w-6xl px-4 lg:px-6'>
        <div className='rounded-[20px] border border-cyan-200 bg-cyan-50/60 p-6 shadow-sm lg:p-8'>
          ...
          <h2 className='mt-1 text-2xl font-bold text-slate-950'>
            {locale === 'cn' || locale === 'tw'
              ? '先看榜单，再决定是继续看 n8n 替代方案还是切到相邻入口'
              : 'Start with the ranking, then decide whether to keep comparing n8n alternatives or switch to an adjacent path'}
          </h2>
          <p className='mt-2 max-w-3xl text-sm leading-6 text-slate-600'>
            {locale === 'cn' || locale === 'tw'
              ? '如果自动化控制和可维护性已经是明确目标，先收紧 shortlist 往往比继续横向浏览更多页面更有效。'
              : 'If automation control and maintainability are already the goals, narrowing the shortlist first is usually better than continuing to browse more pages horizontally.'}
          </p>
            ...
            {[
              ...
              },
              {
                ...
                desc:
                  locale === 'cn' || locale === 'tw'
                    ? '重新确认你要的是控制感还是最快连接器。'
                    : 'Re-check whether you need control or the fastest connector.',
              },
              {
                href: '/guides/make-alternatives-comparison',
                title: locale === 'cn' || locale === 'tw' ? 'Make 替代方案对比' : 'Make alternatives comparison',
                desc:
                  locale === 'cn' || locale === 'tw'
                    ? '当你更看重可视化编排。'
                    : 'Useful when visual orchestration is the priority.',
              },
              {
                href: '/guides/zapier-alternatives-comparison',
                ...
                desc:
                  locale === 'cn' || locale === 'tw'
                    ? '如果你更关心上手速度。'
                    : 'Better when setup speed matters most.',
              },
            ].map((item) => (
              <TrackableCtaLink
                key={item.href}
                href={item.href}
                ctaId={`n8n_ranking_${item.href.split('/').pop()}`}
                ctaLabel={item.title}
                pageType='guide'
