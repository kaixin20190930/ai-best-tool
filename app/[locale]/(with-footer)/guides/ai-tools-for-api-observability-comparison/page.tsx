    ...
    <>
        ...
        <div>
          ...
          <p className='mt-2 text-sm leading-6 text-slate-600'>
            {locale === 'cn' || locale === 'tw'
              ...
              : 'The API observability comparison page has been rechecked against real logs and cost tracking scenarios.'}
          </p>
        </div>
        <div>
          <p className='text-xs font-semibold uppercase tracking-wide text-cyan-700'>
          ...
          <p className='mt-2 text-sm leading-6 text-slate-700'>
            {locale === 'cn' || locale === 'tw'
              ? '保留索引，继续强调日志、成本和质量追踪。'
              : 'Keep it indexable and keep emphasizing logs, cost visibility, and quality tracking.'}
          </p>
        </div>
        <div>
          <p className='text-xs font-semibold uppercase tracking-wide text-cyan-700'>
          ...
          <p className='mt-2 text-sm leading-6 text-slate-700'>
            {locale === 'cn' || locale === 'tw'
              ? '补一个真实生产日志与告警配置案例。'
              : 'Add one real production log and alert setup case.'}
          </p>
        </div>
      </section>
      ...
      <section className='mx-auto mt-8 max-w-6xl px-4 lg:px-6'>
        <div className='rounded-[20px] border border-cyan-200 bg-cyan-50/60 p-6 shadow-sm lg:p-8'>
          ...
          <h2 className='mt-1 text-2xl font-bold text-slate-950'>
            {locale === 'cn' || locale === 'tw'
              ? '先回榜单，再决定是否继续看可观测工具或切到相邻入口'
              : 'Return to the ranking first, then decide whether to keep comparing observability tools or switch to an adjacent path'}
          </h2>
          <p className='mt-2 max-w-3xl text-sm leading-6 text-slate-600'>
            {locale === 'cn' || locale === 'tw'
              ? '如果你已经明确要做日志、成本和质量治理，先收紧 shortlist 往往比继续横向浏览更多页面更有效。'
