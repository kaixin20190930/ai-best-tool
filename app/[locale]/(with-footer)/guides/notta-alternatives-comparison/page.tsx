      ...
      <section className='mx-auto mt-8 max-w-6xl px-4 lg:px-6'>
        <div className='rounded-[20px] border border-cyan-200 bg-cyan-50/60 p-6 shadow-sm lg:p-8'>
          ...
          <h2 className='mt-1 text-2xl font-bold text-slate-950'>
            {locale === 'cn' || locale === 'tw'
              ? '先看榜单，再决定是继续看 Notta 替代还是切到相邻入口'
              : 'Start with the ranking, then decide whether to keep comparing Notta alternatives or switch to an adjacent path'}
          </h2>
          <p className='mt-2 max-w-3xl text-sm leading-6 text-slate-600'>
            {locale === 'cn' || locale === 'tw'
              ? '如果你已经明确要做会议转写或录音整理，先收紧 shortlist 往往比继续横向浏览更有效。'
              : 'If meeting transcription or recording cleanup is already the goal, narrowing the shortlist first is usually better than continuing to browse horizontally.'}
          </p>
            ...
            {[
              {
                ...
                desc:
                  locale === 'cn' || locale === 'tw'
                    ? '先收窄到更值得试用的记录候选。'
                    : 'Narrow to the most trial-worthy capture candidates first.',
              },
              {
                href: '/guides/ai-tools-for-meeting-notes-comparison',
              ...
              {
                href: '/guides/descript-alternatives-comparison',
                title: locale === 'cn' || locale === 'tw' ? 'Descript 替代方案' : 'Descript alternatives',
                desc:
                  locale === 'cn' || locale === 'tw'
                    ? '如果你要把转写和音频整理一起看。'
                    : 'A better path when transcription and audio cleanup are both in scope.',
              },
              {
                ...
                desc:
                  locale === 'cn' || locale === 'tw'
                    ? '如果你的重点开始偏语音和音频。'
                    : 'Useful when the focus shifts more toward voice and audio workflows.',
              },
            ].map((item) => (
              <TrackableCtaLink
                key={item.href}
                href={item.href}
                ctaId={`notta_ranking_${item.href.split('/').pop()}`}
                ctaLabel={item.title}
                pageType='guide'
