...
export default function MobileTable() {
  ...
  return (
    <div className='mb-10 space-y-1 lg:hidden'>
      <div className='flex h-8 items-center justify-between rounded bg-slate-900 px-3 text-xs font-bold text-slate-100'>
        <div>{t('website')}</div>
        <div>{t('submission')}</div>
      </div>
      <div className='flex flex-col gap-1'>
        {STARTUP_LIST.map((item) => (
          <div key={item.DA} className='theme-surface flex min-h-[99px] items-center justify-between rounded p-3'>
            <div className='flex flex-1 flex-col'>
              <div className='mb-[9px] text-sm text-slate-800'>
            ...
            <a
              ...
              target='_blank'
              rel='noreferrer'
              className='flex-center h-[75px] w-9 rounded-[4px] border border-slate-300 hover:bg-slate-100'
            >
