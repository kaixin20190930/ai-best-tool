...
assert(pipeline.includes("slug: 'glean'"));
assert(releaseAudit.includes("glean: ['glean'"));
assert.equal(audit.reviewedAt, '2026-09-09');
assert.equal(audit.publishNotBefore, '2026-09-15');
...
for (const fact of [
  ...
  /source permissions/,
  /company-reported.*\$200 million ARR/i,
  /不能修复源系统中错误的权限/,
  /没有公开稳定的自助价格/,
]) assert.match(factText, fact);

assert(!/guarantees? accuracy|guarantees? ROI|best enterprise search|public per-seat price/i.test(factText));
...
if (audit.status === 'released') {
  ...
  const duplicateRelease = spawnSync(
    'tsx',
    ['scripts/candidate-release-pipeline.ts', '--candidate=glean', '--phase=release', '--as-of=2026-09-20'],
...
console.log('PASS Glean: enterprise search, permissions, deployment, pricing and noindex gates');
