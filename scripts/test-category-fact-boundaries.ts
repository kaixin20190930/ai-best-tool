...
assert(source.includes("'Category navigation checked'"));
assert(source.includes('see each tool’s source dates for product facts.'));
assert(source.includes('data-category-fact-boundary'));
assert(source.includes('data-category-decision-links'));
assert(
  source.includes('Pricing, features, limits, evidence dates, and decision status come from the linked tool page.'),
);
...
assert.equal(
  new Set(representativeHrefs).size,
  representativeHrefs.length - 1,
  'Only the intentional Notta cross-category reuse is allowed',
);
