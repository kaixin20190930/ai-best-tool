...
const officialEvidencePaths = new Set([
  '/ai/lindy',
  '/ai/fathom',
  '/ai/the-graph',
  '/ai/dune',
    ...
    JSON.stringify(
      {
        ...
        officialEvidence: rows.filter((row) => officialEvidencePaths.has(row.path) && row.officialEvidence).length,
      },
      null,
      2,
    ),
  ...
  if (
    ...
    rows.some(
      (row) =>
        ...
        !row.evidenceSignal ||
        !row.actionSignal ||
        (officialEvidencePaths.has(row.path) && !row.officialEvidence),
    )
  ) {
