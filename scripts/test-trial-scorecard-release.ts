...
assert.match(actions, /TRIAL_WINDOW_ACTIVE/);
assert.match(actions, /canCompleteTrial\(String\(scorecard\.ends_at\), completedAt\)/);
assert.match(actions, /\.lte\('ends_at', completedAt\.toISOString\(\)\)/);
...
assert.match(listPage, /SELECT id, name, title, features FROM tools/);
assert.match(listPage, /trialTemplate/);
assert.match(creator, /Creating trial/);
assert.match(creator, /Suggested outcome and checks loaded/);
assert.match(creator, /template\.checks\.slice\(0, 5\)/);
assert.match(scorecard, /Saving final decision/);
assert.match(scorecard, /pendingCount > 0/);
assert.match(scorecard, /!completionAvailable/);
assert.match(scorecard, /observation window is still active/);
