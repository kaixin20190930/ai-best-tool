...
async function verifyRelationships(): Promise<VerificationResult> {
  ...
  try {
    ...
    const { data: tools, error: toolsError } = await supabase
      .from('tools')
      .select('name, category_id, tags');
    
    if (toolsError) {
      return {
        passed: false,
        message: `查询工具失败: ${toolsError.message}`,
      };
    }
    ...
    if (categoriesError) {
      return {
        passed: false,
        message: `查询分类失败: ${categoriesError.message}`,
      };
    }
...
async function main() {
  ...
  try {
    ...
    if (failed === 0) {
      ...
      console.log('\n✅ 数据迁移成功完成');
      console.log('\n下一步:');
      console.log('  1. 在 Supabase Dashboard 中查看数据');
      console.log('  2. 更新应用代码以使用数据库数据');
      console.log('  3. 配置 Row Level Security (RLS) 策略');
    } else {
